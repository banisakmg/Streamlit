...
def ai_ask(
    ...
    if api_key is None or api_url is None:
        if "idToken" in globals():
            ...
            return (
                "Login on the Functions tab for limited demo usage, or sign up for a "
                "free Mistral AI account at https://console.mistral.ai/ and add your own api_key."
            )

    if not isinstance(temperature, (float, int)) or not (
        0 <= float(temperature) <= 2
    ):
        return "Error: temperature must be a float between 0 and 2 (inclusive)"

    if not isinstance(max_tokens, (float, int)) or not (
        5 <= float(max_tokens) <= 5000
    ):
